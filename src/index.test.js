...
test("The Ship 5 coordinates are smaller than his length in start [0,10] and final [1,10]", () => {
  expect(() => theGameboard.placeShip(theShip5, [0,10], [1,10])).toThrow(Error);
});

test("The Ship 5 in coordinate start [1,4] and final [-2,4] cannot be possible", () => {
  expect(() => theGameboard.placeShip(theShip5, [1, 4], [-2, 4])).toThrow(Error);
});

test("The Ship 5 can not be put diagonally in coordinate start [7,7] and final [10,10]", () => {
  expect(() => theGameboard.placeShip(theShip5, [7, 7], [10, 10])).toThrow(Error);
});
...
//   let theGrid = theGameboard.getGrid;
//   expect(theGrid[3][1]).toBe(3);
//   expect(theGrid[4][1]).toBe(3);
//   expect(theGrid[5][1]).toBe(3);
//   expect(theGrid[6][1]).toBe(3);
// });

// test("The Ship 6 can not be put in coordinates already in use in coordinate start [5,5] and final [5,5]", () => {
//   expect(() => theGameboard.placeShip(theShip5, [2, 5], [2, 8])).toThrow(Error);
// });

// test("The Ship 6 error of putting him in an existing coordinate erase the already put pointers", () => {
//   let theGrid = theGameboard.getGrid;
//   expect(theGrid[2][5]).toBe(0);
//   expect(theGrid[2][6]).toBe(2);
...
//   theGameboard.placeShip(theShip6,[3,1],[6,1]);
//   let theGrid = theGameboard.getGrid;
//   expect(theGrid[3][1]).toBe(3);
//   expect(theGrid[4][1]).toBe(3);
//   expect(theGrid[5][1]).toBe(3);
